...
void IOS_HandleJoystickAxes(float axes[NUM_AXIS_CODES]) {
  // Map Y axis to forward/back (up/down on left thumbstick)
  if ([JoystickInputHolder shared].axisY < 0) {
    axes[AXIS_CODE_PAD_LTHUMB_UP] = fabs([JoystickInputHolder shared].axisY);
    axes[AXIS_CODE_PAD_LTHUMB_DOWN] = 0;
  } else {
    axes[AXIS_CODE_PAD_LTHUMB_UP] = 0;
    axes[AXIS_CODE_PAD_LTHUMB_DOWN] = [JoystickInputHolder shared].axisY;
  }
  
  // Map X axis to side movement (left/right on left thumbstick)
  float xAxis = [JoystickInputHolder shared].axisX * (guiCapture ? 1 : -1.0);
  if (xAxis < 0) {
    axes[AXIS_CODE_PAD_LTHUMB_LEFT] = fabs(xAxis);
    axes[AXIS_CODE_PAD_LTHUMB_RIGHT] = 0;
  } else {
    axes[AXIS_CODE_PAD_LTHUMB_LEFT] = 0;
    axes[AXIS_CODE_PAD_LTHUMB_RIGHT] = xAxis;
  }
  
  // translate axis values to button states - might be for directional input for menu?
  uint8_t buttonstate = Joy_XYAxesToButtons(xAxis, [JoystickInputHolder shared].axisY);
